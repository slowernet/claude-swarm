...
```
Plan -> Scouts -> Gate -> Workers -> Merge -> Review
  ^                 |        ^                   |
...
- **Workers** (waves, ≤4 concurrent): implement inside a fixed file scope, consulting the store before exploring anything. Test-first, watching each test fail before making it pass. They never spawn subagents, and they report one of four statuses with the test output pasted, not summarized. A worker that discovers the plan is wrong stops and reports instead of improvising. When the wave is in, the lead runs the full suite in-session before anything is reviewed.
- **Merge** (only when a worker ran in an isolated worktree): one agent applies its changes to the shared tree, resolving conflicts in favour of the decision entries, fixes the seams the merge introduced, recaptures the diff so review reads the merged work, and runs the full suite. With no worktree there is nothing to merge, and cross-worker seams fall to the reviewer's conventions and simplicity lenses.
...
- Workers: may sit one tier below the lead, protected by the reviewer above them
...
The skill runs on either of Claude Code's spawn substrates: plain subagents, or teammates ([agent teams](https://code.claude.com/docs/en/agent-teams)). One rule makes it substrate-proof: no report rides on an agent's final text, because a finished agent's final text may never reach the lead. Durable output goes in the store and summaries go by SendMessage before the agent finishes, and gating a risky worker uses the same mechanism on both substrates: the worker messages its plan and waits for the lead's reply before it writes anything.

## Files

- `SKILL.md` — the lead's playbook: phases, sizing, failure handling, model policy
- `references/entry.md` — the knowledge-entry format, loaded by every agent that writes one
- `references/templates.md` — the four-status report contract and verbatim prompt templates for scout, plan reviewer, worker, integrator, code reviewer
...
- Spawned agents do not survive a session resume. The store and task list do, so the lead respawns from them.
...
- The workflow has not yet been burned in against a large real task.
